...
import type { Message, VetStackParamList } from '../../types';

type ChatRouteProp = RouteProp<VetStackParamList, 'VetChatConversation'>;
type NavigationProp = NativeStackNavigationProp<VetStackParamList>;

export default function VetChatConversationScreen() {
  const navigation = useNavigation<NavigationProp>();
  const route = useRoute<ChatRouteProp>();
  const { threadId, participantName } = route.params;

  const { user } = useAuth();
  const { messages, loading, sending, sendMessage } = useChatMessages(threadId);
    ...
    <View className="flex-1 bg-beige">
      ...
      <View className="px-5 pt-14 pb-3 flex-row items-center bg-beige border-b border-gray-100">
        ...
        <View className="w-9 h-9 rounded-full bg-primary/15 items-center justify-center mr-3">
          <Ionicons name="person" size={16} color="#71924F" />
        </View>
        <View className="flex-1">
          <Text className="text-base font-semibold text-dark">{participantName}</Text>
          <Text className="text-[10px] text-grey">Confidential patient communication</Text>
        </View>
      </View>
