...
import type { ChatThread, VetStackParamList } from '../../types';
  ...
  function renderThread({ item }: { item: ChatThread }) {
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.7}
      >
        <Avatar uri={item.participant.avatar_url} name={item.participant.name} size={48} />
    ...
    return (
      <View className="flex-1 items-center justify-center px-10 pt-20">
        ...
        <Text className="text-sm text-grey text-center mt-2">
          Pet owners will reach out after booking appointments.
        </Text>
      </View>
    );
  }
  ...
  return (
    <View className="flex-1 bg-beige">
      ...
      <View className="px-5 pt-14 pb-2">
        <Text className="text-2xl font-bold text-heading">Messages</Text>
        <Text className="text-xs text-grey mt-1">Patient conversations are confidential</Text>
      </View>
