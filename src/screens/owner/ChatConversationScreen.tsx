...
import type { Message, OwnerStackParamList } from '../../types';

type ChatRouteProp = RouteProp<OwnerStackParamList, 'ChatConversation'>;
...
export default function ChatConversationScreen() {
  ...
  async function handleSend() {
    ...
    const text = input;
    setInput(''); // Clear immediately for responsive UX
    ...
    <View className="flex-1 bg-beige">
      ...
      <View className="px-5 pt-14 pb-3 flex-row items-center bg-beige border-b border-gray-100">
        ...
        <View className="w-9 h-9 rounded-full bg-primary/15 items-center justify-center mr-3">
          <Ionicons name="medical" size={16} color="#71924F" />
        </View>
        <View className="flex-1">
          <Text className="text-base font-semibold text-dark">{participantName}</Text>
          <Text className="text-[10px] text-grey">Messages are private & secure</Text>
        </View>
      </View>
